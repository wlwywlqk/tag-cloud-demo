...
export class TagCloud {
    ...
    private async handleTag(tag: Tag): TagData {
        const { minTagWeight, maxTagWeight } = this;
        const { minFontSize, maxFontSize, angleCount, angleFrom, angleTo } = this.options;
        const { text, weight, angle: maybeAngle, color: maybeColor } = tag;


        const diffWeight = maxTagWeight - minTagWeight;
        const fontSize = diffWeight > 0
            ? Math.round(minFontSize + (maxFontSize - minFontSize) * ((weight - minTagWeight) / diffWeight))
            : Math.round((maxFontSize + minFontSize) / 2 );

        const randomNum = Math.random() * angleCount >> 0;
        const angle = maybeAngle === undefined
            ? angleFrom + (randomNum / (angleCount - 1) * (angleTo - angleFrom))
            : maybeAngle;
        
        const color = maybeColor === undefined
    ...
    private async getTagPixels({ text, angle, fontSize, color, }: { text: string, angle: number, fontSize: number, color: string }): Promise<void> {
        ...
        const { width }: TextMetrics = this.ctx.measureText(text);
        console.log(this.ctx.measureText(text), fontSize)
        ...
        if (sinTheta > 0) {
            ...
            const len = width * -sinTheta;
            const x =  len * sinTheta;
            const y =  len * cosTheta + height;
    ...
    private async loadMaskImage($maskImage: HTMLImageElement) {
        const { width, height, opacityThreshold, lightThreshold, pixelRatio, debug } = this.options;
        ...
        while (pixelCount--) {
            const outerOffset = pixelY * pixelRatio * dataXLength + (pixelX * pixelRatio << 2);
            ...
            outer: while (y < yLength) {
                ...
                while (x < xLength) {
                    ...
                    if (debug) {
                        ...
                        this.ctx.fillRect(pixelX * pixelRatio, pixelY * pixelRatio, pixelRatio - 0.2, pixelRatio - 0.2);
                    }
